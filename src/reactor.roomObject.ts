...
export interface RawReactor extends RawObject {
	type: "reactor";
	store: any;
	launchTime?: number | null;
	user?: RawId<User> | null;
	storeCapacityResource: ResourceStoreCapacity;
}

export interface Reactor extends RoomObject, _HasId {
	toString(): string;
}
...
export default function(config: ServerConfig) {
	...
	if (config.backend) {
		...
		const utils = require.main!.require('@screeps/backend/lib/utils.js') as typeof import('@screeps/backend/lib/utils.js');
		const previousRespawn = utils.respawnUser;
		utils.respawnUser = async function(userId: UserId) {
	...
	if(config.engine) {
		config.engine.registerCustomObjectPrototype<RawReactor>('reactor', 'Reactor', {
			prototypeExtender (prototype: GameOf<RawReactor>, scope, {utils}) {
				const data = (id: string): RawReactor => {
					if (!scope.runtimeData.roomObjects[id]) {
						throw new Error("Could not find an object with ID " + id);
					}
					return scope.runtimeData.roomObjects[id] as RawReactor;
				};
	...
	if(config.cronjobs) {
		config.cronjobs.genReactors = [60, async () => {
			const utils = require.main!.require('@screeps/backend/lib/utils.js') as typeof import('@screeps/backend/lib/utils.js');
			...
			for(const room of rooms) {
				const reactor = roomObjects.find((o: RawObject) => o.type === 'reactor' && o.room === room._id) as RawReactor | undefined;
				if(reactor) {
					continue;
				}
				console.log(`No reactor in room ${room._id}`);
				const wallObjects = roomObjects.filter(
					(o: RawObject) => o.room === room._id && (o.type === "source" || o.type === "mineral" || o.type === "controller")
				);
				const roomTerrain = await db['rooms.terrain'].findOne({room: room._id});
				const terrain = roomTerrain?.terrain;
				if(!terrain) {
					console.log(`No terrain in room ${room._id}`);
					continue;
				}
